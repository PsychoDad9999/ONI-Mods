...
namespace OniMods.FoodDiagnosticsFix
{
    ...
    static class FoodDiagnostic_CheckEnoughFood_Patch
    {
        ...
        /// </summary>        
        static bool Prefix(FoodDiagnostic __instance, ref ColonyDiagnostic.DiagnosticResult __result, float ___trackerSampleCountSeconds)
        {            
        ...
        private static float GetRequiredFoodPerCycleByAttributeModifier(List<MinionIdentity> dupes)
        {
            ...
            {               
                ...
                {
                    float caloriesPerSecond = Db.Get().Amounts.Calories.Lookup(dupe).GetDelta();
